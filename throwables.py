...
class ThrownGrenade:
    ...
    def update(self, platforms):
        ...
        if self.state == "ACTIVE":
            ...
            for p in platforms:
                if not (self.x + self.size <= p.x or self.x >= p.x + p.w or 
                        self.y + self.size <= p.y or self.y >= p.y + p.h):
            ...
            self.explosion_frame = min(elapsed // self.frame_duration, 
    ...
    def draw(self, screen):
        if self.state == "ACTIVE":
            rotated = pygame.transform.rotate(self.weapon_ref.images["grenade"], self.rotation)
            rect = rotated.get_rect(center=(self.x + self.size // 2, self.y + self.size // 2))
            screen.blit(rotated, rect)
        
        elif self.state == "EXPLODING":
            explosion_img = self.weapon_ref.images["explosion"][self.explosion_frame]
            rect = explosion_img.get_rect(center=(self.x + self.size // 2, self.y + self.size // 2))
    ...
    def check_collision(self, player):
        ...
        if self.state == "EXPLODING" and player.player not in self.hit_players:
            ...
            explosion_radius = 80
            hit = not (
                player.x + player.w <= self.x - explosion_radius or
                player.x >= self.x + self.size + explosion_radius or
                player.y + player.h <= self.y - explosion_radius or
                player.y >= self.y + self.size + explosion_radius
            )
            if hit:
